...
function Propose ({ isMember, members }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <>
      ...
      {isOpen && (
        <Modal
          header={t<string>('Propose a board motion')}
          onClose={toggleOpen}
          size='large'
        >
          <Modal.Content>
            <Modal.Columns hint={t<string>('The board account for the proposal. The selection is filtered by the current members.')}>
              <InputAddress
                filter={members}
            ...
            <Modal.Columns hint={t<string>('The desired threshold. Here set to a default of 50%+1, as applicable for general proposals.')}>
              <InputNumber
                className='medium'
                help={t<string>('The minimum number of board votes required to approve this motion')}
