...
function Vote ({ electionsInfo }: Props): React.ReactElement<Props> | null {
  ...
  useEffect((): void => {
    accountId && api.derive.boardVirginia.votesOf(accountId).then(({ votes }): void => {
      setDefaultVotes(
        votes
          .map((a) => a.toString())
        ...
        >
          <Modal.Content>
            ...
            >
              <InputAddressMulti
                available={available}
                availableLabel={t<string>('board candidates')}
                defaultValue={defaultVotes}
                help={t<string>('Select and order board candidates you wish to vote for.')}
                maxCount={MAX_VOTES}
                onChange={setVotes}
