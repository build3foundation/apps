...
function SubmitCandidacy ({ electionsInfo }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <>
      {isOpen && (
        <Modal
          header={t<string>('Submit your board candidacy')}
          onClose={onClose}
          size='large'
        >
          <Modal.Content>
            <Modal.Columns hint={t<string>('This account will appear in the list of candidates. With enough votes in an election, it will become either a runner-up or a board member.')}>
              <InputAddress
